...
#'    and a known set of forbidden paths, into a new graph that does not allow any forbidden path as
#'    part of its solutions. This algorithm should only be used when there is certainty that no forbidden
#'    path is a sub-path (or contains a sub-path) of another forbidden path.
#'
...
#'    arc. The first two columns must be named \code{from} and \code{to}, and can be of any data type. No cells can
#'    be \code{NULL} or \code{NA}.
...
#'    thought as an arc that should not exist in the first place. Also, no forbidden path can be sub-path
#'    (or contain a sub-path) of another forbidden path. The columns names are not relevant.
...
#'
#' @return A new graph, generated following Villeneuve's prodcedure, in which no path includes one of the
#'    forbidden subpaths. The graph is returned in a data frame format, where each row represents a directed
#'    arc. However, regardless of the data type of the original graph, nodes on the new graph are of type
